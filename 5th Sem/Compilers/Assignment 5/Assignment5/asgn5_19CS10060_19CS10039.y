...
%token AUTO RESTRICT REGISTER BREAK CASE CHAR CONST CONTINUE DEFAULT DO DOUBLE ELSE ENUM EXTERN FLOAT FOR GOTO IF INLINE INT LONG RETURN SHORT SIGNED SIZEOF STATIC STRUCT SWITCH TYPEDEF UNION UNSIGNED VOID VOLATILE WHILE BOOL COMPLEX IMAGINARY 
...
%token DOT ARROW INCREMENT DECREMENT BITWISE_AND MUL PLUS MINUS BITWISE_NOT EXCLAIM DIV MOD LEFTSHIFT RIGHTSHIFT LESSTHAN GREATERTHAN
%token LESSTHANEQUAL GREATERTHANEQUAL EQUALEQUAL NOTEQUAL BITWISE_XOR BITWISE_OR AND OR
%token QUESTION COLON SEMICOLON ELLIPSIS ASSIGN 
%token MULTIPLYASSIGN DIVIDEASSIGN MODASSIGN ADDASSIGN SUBTRACTASSIGN LEFTSHIFTASSIGN RIGHTSHIFTASSIGN ANDASSIGN XORASSIGN ORASSIGN 
%token COMMA HASH 
...
relational_expression: shift_expression   { $$=$1; }            
	...
	| relational_expression LESSTHANEQUAL shift_expression			 //similar to above, check compatible types,make new lists and emit
	{
		if(!compareSymbolType($1->loc, $3->loc)) 
		{
			cout << "Type Error in Program"<< endl;
...
iteration_statement: WHILE W ROUND_BRACKET_OPEN X changetable M expression ROUND_BRACKET_CLOSE M loop_statement      //while statement
	...
	| DO D M loop_statement M WHILE ROUND_BRACKET_OPEN expression ROUND_BRACKET_CLOSE SEMICOLON      //do statement
	{
		...
		$$->nextlist = $8->falselist;                       //if false, move out 
		loop_name = "";
	}
	| DO D CURLY_BRACKET_OPEN M block_item_list_opt CURLY_BRACKET_CLOSE M WHILE ROUND_BRACKET_OPEN expression ROUND_BRACKET_CLOSE SEMICOLON      //do statement
	{
		...
		$$->nextlist = $10->falselist;                       //if false, move out 
		loop_name = "";
	}
	| FOR F ROUND_BRACKET_OPEN X changetable declaration M expression_statement M expression N ROUND_BRACKET_CLOSE M loop_statement     //for loop
	{
		...
		convertIntToBool($8);  
		backpatch($8->truelist, $13);	//if true, go to M2
...
function_definition:declaration_specifiers declarator declaration_list_opt changetable CURLY_BRACKET_OPEN block_item_list_opt CURLY_BRACKET_CLOSE  
	{
		...
		table_count = 0;
		label_table.clear();
		changeTable(globalST);                     //at the end, the table is global Symbol table
	};
