...
symbolTable* parST;                                                                                   // denotes the Parent of the current Symbol Table
...
symbolTable::symbolTable(string name)                                                                    // constructor for a symbol table
{
    (*this).name=name;                                                                             // Initialize the name of the symbol table
    count=0;                                                                                       // Put count of number of temporary variables as 0
}

sym* symbolTable::lookup(string name)                                                 // Lookup an symbol in the symbol table, whether it exists or not
{
    sym* symbol;
	...
	//if not found alreday create new entry in table and return the address to that new entry
    if(ST == this and !ptr){
        symbol = new sym(name);
...
void symbolTable::update()                                                                            // Update the symbol table and the offsets in it
{
    list<symbolTable*> tb;                                                                            // list of tables
    int off;
    ...
    list<symbolTable*>::iterator it1;                                                                   // list iterator for the nested tables
    it1=tb.begin();
...
void symbolTable::print()                                                                                // print a symbol table
{
    int next_instr=0;
    list<symbolTable*> tb;                                                                               // list of tables
...
//***the Quad Array Class functions***/
...
//Emits TAC and add it to the Quad Array 
...
/***functions required for TAC generation ***/
...
/***function for debugging and printing **/
